...
-- How many unique cutomers we have? >> 155
SELECT 
	COUNT(DISTINCT customer_id)
FROM
	retail_sales;

-- unique categories we have? >> 3 (Electronics, Clothing, Beauty)
SELECT 
	DISTINCT category
FROM
	retail_sales;
...
FROM (
	SELECT 
		EXTRACT(YEAR FROM sale_date) AS year,   -- in MySQL only write> YEAR(sale_date) 
...
SELECT      -- we don't need rank column
	year,
	month,
	avg_total_sale
FROM (
...
SELECT *, 
	CASE
		WHEN EXTRACT(HOUR FROM sale_time) < 12 THEN 'Morning'                --SELECT EXTRACT(HOUR FROM CURRENT_TIME)
